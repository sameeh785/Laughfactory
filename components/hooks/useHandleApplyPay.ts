...
export const useHandleApplyPay = (disabled: boolean) => {
  ...
  const validateMerchant = async (validationURL: string): Promise<any> => {
    try {
      ...
      const response = await fetch("/api/applePay/validateMerchant", {
        method: "POST",
        headers: {
    ...
    try {
      ...
      session.onvalidatemerchant = async (event: any) => {
        try {
          ...
          if (merchantSession.status) {
            session.completeMerchantValidation({
              merchantIdentifier: merchantSession?.data?.merchantIdentifier,
              displayName: merchantSession?.data?.displayName,
              initiative: "web",
              initiativeContext: "https://ld15rlvs-3000.inc1.devtunnels.ms/",
            });
          }
        } catch (error) {
